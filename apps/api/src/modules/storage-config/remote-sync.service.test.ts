...
vi.mock("@/modules/settings/tmdb-key.query", () => ({
  getSettingsTmdbApiKey,
}));

vi.mock("./remote-storage.service", () => ({
  remoteStorageService: { listDirectories, listFiles, ensureDirectory, moveFile },
}));
...
describe("remote-sync.service", () => {
  ...
  beforeEach(() => {
    ...
    getSettingsTmdbApiKey.mockReset().mockResolvedValue("tmdb-key");
  ...
  it("runRemoteSync requires TMDB key and enabled storage", async () => {
    getSettingsTmdbApiKey.mockResolvedValue(null);
    await expect(runRemoteSync(user.id)).rejects.toThrow(/TMDB API key/);

    getSettingsTmdbApiKey.mockResolvedValue("key");
