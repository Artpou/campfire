...
import { requireMediaExists } from "./media.guard";
...
export const mediaRoutes = MediaService.createRouter()
  ...
  .get("/:id", zValidator("param", mediaIdParamDto), requireMediaExists, async (c) => {
  ...
  .delete("/:id/review", zValidator("param", mediaIdParamDto), requireMediaExists, async (c) => {
    const { id } = c.req.valid("param");
    return c.json(await c.var.service.deleteReview(Number(id)));
  })
  .patch(
    "/:id/progress",
    zValidator("param", mediaIdParamDto),
    requireMediaExists,
    zValidator("json", updateProgressDto),
    async (c) => {
      const { id } = c.req.valid("param");
