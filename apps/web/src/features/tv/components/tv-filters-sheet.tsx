...
function countActive(value: TvFiltersValue): number {
  let count = 0;
  if (value.first_air_date_gte || value.first_air_date_lte) count++;
...
export function TvFiltersSheet({ value, onChange }: TvFiltersSheetProps) {
  ...
  const [draft, setDraft] = useState<TvFiltersValue>(value);
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="w-full sm:max-w-md flex flex-col">
        <SheetHeader>
          ...
          <SheetDescription>
            <Trans>Refine the list of TV shows by combining several criteria.</Trans>
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="flex-1 overflow-y-auto px-4 space-y-6">
          <div className="space-y-2">
            <Label>
              <Trans>First air date</Trans>
            </Label>
            <div className="grid grid-cols-2 gap-2">
              <Input
                type="date"
                label={<Trans>From</Trans>}
                id="first-air-date-gte"
                value={draft.first_air_date_gte ?? ""}
                onChange={(e) =>
                  setDraft((prev) => ({
                    ...prev,
                    first_air_date_gte: e.target.value || undefined,
                  }))
                }
              />
              <Input
                ...
                id="first-air-date-lte"
                value={draft.first_air_date_lte ?? ""}
                onChange={(e) =>
                  setDraft((prev) => ({
                    ...prev,
                    first_air_date_lte: e.target.value || undefined,
                  }))
                }
              />
            </div>
          ...
          <div className="space-y-2">
            <Label htmlFor="tv-original-language">
            ...
            >
              <SelectTrigger id="tv-original-language" className="w-full">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {LANGUAGES.map((lang) => (
          ...
          <div className="space-y-2">
            <Label htmlFor="tv-keyword-search">
              <Trans>Keyword</Trans>
            </Label>
            <div className="space-y-2">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <Label>
                <Trans>Episode runtime (minutes)</Trans>
              </Label>
