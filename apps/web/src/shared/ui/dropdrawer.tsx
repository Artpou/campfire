...
function DropDrawerContent({
  ...
  if (isMobile) {
    return (
      ...
      >
        <DrawerContent data-slot="drop-drawer-content" className={cn("max-h-[90vh]", className)} {...props}>
          {activeSubmenu ? (
            <>
              <DrawerHeader>
...
function DropDrawerGroup({
  ...
  const childrenWithSeparators = React.useMemo(() => {
    ...
    return filteredChildren.flatMap((child, index) => {
      ...
      return [child, <div key={`separator-${index}`} className="bg-border h-px" aria-hidden="true" />];
    });
  }, [children, isMobile]);
  ...
  return (
    <div data-drop-drawer-group data-slot="drop-drawer-group" role="group" className={className} {...props}>
      {children}
    </div>
  );
}
